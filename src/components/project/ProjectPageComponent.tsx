...
export function ProjectCard(props: { project: Project, className?: string }) {
    return (
        ...
        <div className={`h-full group ${props.className}`}>
            <div className='h-full bg-cover bg-center bg-no-repeat rounded-3xl group' style={{ backgroundImage: `url(${props.project.image})` }}>
                {/* explanation card */}
                <a href="#" className="group-hover:bg-opacity-90 group-hover:block h-full bg-accent-secondary bg-opacity-0 rounded-3xl trasition-all duration-500">
                    <div className="flex flex-col justify-center h-full p-5 opacity-0 group-hover:opacity-100 transition-all duration-500">
                        <h1 className="text-2xl font-bold text-primary">{props.project.title}</h1>
